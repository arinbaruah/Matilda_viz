...
ggplot(data = df_substance_deaths %>% filter(Entity %in% tags) , aes(x = Year,
  ...
  geom_rect(aes(
    xmin = 2020, xmax = 2022,    # X range for the shaded region
    ymin = 0, ymax = 40   # Y range for the shaded region
  ), fill = "steelblue",
     alpha = 0.1) +
  ...
  theme(
        ...
        plot.title    = element_text(size = 26, face = "bold", color = "#2E2A2B"),
        plot.subtitle = element_markdown(size = 11, margin = margin(b = 10)),
        plot.caption  = element_markdown(size = 7,  margin = margin(t = 10)),
