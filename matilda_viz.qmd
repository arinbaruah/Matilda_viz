...
age_exclude <- c("0-4","all","5-14")
df_substance_deaths_age_long <- df_substance_deaths_age_long %>% filter(!age_group_clean %in% age_exclude)
df_substance_deaths_age_long_grouped <- df_substance_deaths_age_long %>% group_by(age_group_clean) %>% summarise(Total_deaths = sum(deaths))
```
...
pal <- c("#ff6037", "#e6bc00", "#3a9234","#2E86C1")


gr3 <- ggplot(df_substance_deaths_age_long_grouped, aes(id = age_group_clean,area = Total_deaths,fill = Total_deaths)) +
  geom_circlepack() +
  geom_circlepack_text(aes(label = age_group_clean, family = "title", fontface = "bold",color = "#f1fffe"),size = 6) +
  geom_circlepack_text(aes(y = after_stat(y - 200), label = round(Total_deaths/10^6,1), 
                           family = "text",color = "white", 
                           fontface = "bold", lineheight = 0.7),size = 4) +
  ...
  theme(
    ...
    strip.text = element_text(color = pal[1], size = 3),
