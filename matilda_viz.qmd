...
gr3 <- ggplot(df_substance_deaths_age_long_grouped, aes(id = age_group_clean,area = Total_deaths,fill = Total_deaths)) +
  geom_circlepack() +
  geom_circlepack_text(aes(label = age_group_clean, family = "title", fontface = "bold"),color = "#f1fffe",size = 2.5,
                       check_overlap = TRUE, family = "caption", lineheight = 0.9) +
  geom_circlepack_text(aes(y = after_stat(y - 90), label = round(Total_deaths/10^6,1), 
