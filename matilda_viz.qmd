...
gr6 <- ggplot(data = df_overdose_long,aes(x = Year,y = death_rate)) + geom_line() +
  geom_area(alpha = 0.4,aes(fill = drug_type_clean)) + 
  facet_wrap(~drug_type_clean,nrow = 1) + 
  geom_point(data = df_overdose_long %>% filter(Year == 2020),aes(x = Year,y = death_rate),size = 3,color = "#21618C") +
